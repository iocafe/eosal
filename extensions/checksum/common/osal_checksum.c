...
static OS_CONST os_ushort crc_table[] = {
...
****************************************************************************************************
  ...
  The checksum can be also calculated in parts by calling this function multiple times.
  For example:

  char data[6] = "pekka";
  os_ushort checksum = OSAL_CHECKSUM_INIT;
  os_checksum(data, 3, &checksum);
  os_checksum(data + 3, 3, &checksum);
...
{
    os_uchar tmp;
    os_ushort w = OSAL_CHECKSUM_INIT;
    register os_int count;
    ...
    if (append_to_checksum) *append_to_checksum = w;
    return w;
}

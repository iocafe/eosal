...
#if OSAL_NETWORK_INTERFACE == OSAL_OS_NETWORK_INTERFACE
  #include <LwIP.h>
    ...
    os_int n_wifi)
{
  ...
  if (Ethernet.hardwareStatus() == EthernetNoHardware) {
    osal_debug_error("Ethernet shield was not found.  Sorry, can't run without hardware. :(");
    while (true) {
      delay(1); // do nothing, no point running without Ethernet hardware
    }
  }
  if (Ethernet.linkStatus() == LinkOFF) {
    osal_debug_error("Ethernet cable is not connected.");
  }
...
osalStatus osal_are_sockets_initialized(
    void)
{
    return osal_sockets_initialized ? OSAL_SUCCESS : OSAL_STATUS_FAILED;
}

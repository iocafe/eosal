...
****************************************************************************************************
  ...
  @anchor osal_str_to_list

  The osal_str_to_list() converts string representation of MAC or IP address to binary.
    ...
    const os_char *str)
{
    ...
    if (ip_sz >= 16)
    {
        count = osal_str_to_list(buf, 8, str, ':', 16);
        if (count == 8)
        {
            for (i = 0; i < 8; i++)
            {
    ...
    const char* str)
{
    ...
    count = osal_str_to_list(buf, 6, str, '-', 16);
    if (count == 6)
    {
        for (i = 0; i < 6; i++)
        {

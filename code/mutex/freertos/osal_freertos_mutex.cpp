/**
  ...
  @file    mutex/freertos/osal_freertos_mutex.c
  @brief   Mutexes, synchronizing thread access to shared resources.
  @author  Pekka Lehtikoski
  @version 1.0
  @date    26.4.2021

  This file implements mutex related functionality for Arduino/FreeRTOS. Generally mutexes are
  used to synchronize thread access of shared resources. A mutex is created by osal_mutex_create()
...
osalMutex osal_mutex_create(
    void)
{
    /* Inform resource monitor that mutex has been created.
     */
    osal_resource_monitor_increment(OSAL_RMON_MUTEX_COUNT);

    return (osalMutex)xSemaphoreCreateRecursiveMutex();
}
    ...
    osalMutex mutex)
{
    ...
    /* Wait forever
     */
    while (xSemaphoreTakeRecursive((SemaphoreHandle_t)mutex, (TickType_t)1000) != pdTRUE);
}
